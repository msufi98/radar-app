...
function enableZoomFeature() {
    ...
    // Set initial zoom level based on current map zoom
    // Map zoom 8 (when radar loads) should correspond to detail zoom level 8
    // This gives a small starting box size that matches the zoom window
    const currentMapZoom = map.getZoom();
    if (currentMapZoom <= 7) {
        zoomLevel = 4;
    } else if (currentMapZoom <= 9) {
        zoomLevel = 8;
    ...
    console.log(`Initial detail zoom level: ${zoomLevel}× (map zoom: ${currentMapZoom})`);
...
function drawRadarOverlay(latLng, radarCenter, windowSize) {
    ...
    // Find min/max values for color scaling (same as main map)
    let minVal = Infinity;
    let maxVal = -Infinity;
    for (let ray = 0; ray < nrays; ray++) {
        for (let gate = 0; gate < ngates; gate++) {
            const val = reflectivity[ray][gate];
            if (val !== null && val !== undefined && !isNaN(val)) {
                minVal = Math.min(minVal, val);
                maxVal = Math.max(maxVal, val);
            }
    ...
    for (let ray = 0; ray < nrays; ray++) {
        ...
        for (let gate = 0; gate < ngates; gate++) {
            ...
            // Normalize value for color mapping (same as main map)
